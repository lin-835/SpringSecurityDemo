...
public class KaptchaFilter extends UsernamePasswordAuthenticationFilter {
    ...
    private String kaptchaParameter = FORM_KAPTCHA_KEY;//提供默认值的写法，如果不想用默认值，可调用set方法进行设置
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
        ...
        if (!ObjectUtils.isEmpty(verifyCode) && !ObjectUtils.isEmpty(sessionVerifyCode) &&
            ...
            return super.attemptAuthentication(request, response);//这里直接调用父类中方法进行放行就可以了
        }
